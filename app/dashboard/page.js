...
async function getDashboardData(discordId) {
  ...
  if (!achievementLinksError && achievementLinks?.length) {
    ...
    achievements = achievementLinks
      .map((item) => ({
        ...
        achievements: byId.get(item.achievement_id) || null,
      }))
  ...
  if (!titleLinksError && titleLinks?.length) {
    ...
    titles = titleLinks
      .map((item) => ({
        ...
        titles: byId.get(item.title_id) || null,
      }))
  ...
  const { data: leaderboard } = await supabase
    .from('profiles')
    .select('display_name, level, xp, title, avatar_url')
...
export default async function Dashboard() {
  ...
  const xpPercent = Math.min((xp / nextLevel) * 100, 100);

  const achievementItems = achievements.map((item) => ({
    name: item.achievements.achievement_name,
    description: item.achievements.description,
    unlockedAt: item.unlocked_at,
  }));

  const titleItems = titles.map((item) => ({
    name: item.titles.title_name,
    ...
    <main className="dashboardShell">
      ...
      <div className="dashboardLayout">
        ...
        <section className="mainPanel">
          ...
          <div className="welcome" id="profile">
          ...
          <section className="gridStats">
            ...
            <div className="statCard">
              ...
              <em>/ {nextLevel.toLocaleString()}</em>
            </div>
            <div className="statCard">
              <div className="statIcon">◆</div>
              <small>Coins</small>
            ...
            <div className="statCard">
              ...
              <strong>{achievementItems.length}</strong>
              <em>earned</em>
            </div>
          </section>
          ...
          <section className="contentGrid">
            <div className="panel" id="progress">
              ...
              <p style={{ color: '#b9acb7', fontSize: 13 }}>
                Level {level}<span style={{ color: '#716471' }}> • </span>{title}
              </p>
              <div className="xpBar">
              ...
              <p style={{ color: '#716471', fontSize: 12, marginTop: 10 }}>
                {Math.max(nextLevel - xp, 0).toLocaleString()} XP until level {level + 1}
              </p>
            </div>

            <div className="panel" id="achievements">
              <div className="panelHeader"><h2>RECENT ACHIEVEMENTS</h2></div>
              {achievementItems.length ? achievementItems.slice(0, 5).map((achievement, index) => (
                <div className="achievement" key={`${achievement.name}-${achievement.unlockedAt || index}`}>
                ...
                <div className="achievement">
                  <div className="achievementBadge">✦</div>
                  <div className="achievementText">
                    <strong>Your story starts here</strong>
                    <small>Keep exploring Velvet to unlock achievements.</small>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="panel" id="titles">
              <div className="panelHeader"><h2>YOUR TITLES</h2></div>
              {titleItems.length ? titleItems.map((item, index) => (
                <div className="achievement" key={`${item.name}-${index}`}>
                  <div className="achievementBadge">{item.equipped ? '👑' : '✦'}</div>
                  <div className="achievementText">
                ...
                <div className="achievement">
                  ...
                  <div className="achievementText">
                    ...
                    <small>Unlock titles as your Velvet journey continues.</small>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="panel" id="leaderboard">
              ...
              <div className="rankList">
                {leaderboard.length ? leaderboard.map((member, index) => (
                  <div className="rank" key={`${member.display_name || 'member'}-${index}`}>
                    <div className="rankNum">{String(index + 1).padStart(2, '0')}</div>
                    <div>
                      <strong>{member.display_name || 'Velvet Member'}</strong>
                      <small>Level {member.level ?? 1}</small>
                    </div>
                    <b>{(member.xp ?? 0).toLocaleString()}</b>
                  </div>
                )) : (
                  <div className="rank">
                    ...
                    <div>
                      ...
                      <small>Start earning XP to appear here.</small>
                    </div>
                  </div>
                )}
              </div>
